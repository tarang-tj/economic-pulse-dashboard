...
def sahm_rule(unrate: pd.Series) -> pd.DataFrame:
    ...
    moving average of the unemployment rate minus its own minimum over the
    trailing 12 months (inclusive of the current month), triggered when
    ...
    Returns:
        DataFrame indexed like the input, with columns:
          ...
          twelve_mo_min — 12-month trailing min of three_mo_avg (incl. current)
    ...
    three_mo_avg = clean.rolling(window=3, min_periods=3).mean()
    twelve_mo_min = three_mo_avg.rolling(window=12, min_periods=12).min()
    ...
    out["triggered"] = out["gap"] >= SAHM_TRIGGER_THRESHOLD
